...
angular.module('ui.bootstrap.dropdownToggle', []).directive('dropdownToggle', ['$document', '$location', function ($document, $location) {
  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      element.bind('click', function (event) {
          if ($document.find(".open").length !== 0) {
              $document.find(".open").each(function (_, el) {
                  if ($(el) !== element) $(el).removeClass("open");
              });
          }
        ...
        if (!elementWasOpen) {
          ...
          closeMenu = function (event) {
              ...
              var datepicker = $document.find(".datepicker.datepicker-dropdown");
              if (datepicker.is(":visible")) {
                  if (event.pageX >= datepicker.offset().left && event.pageX <= datepicker.offset().left + datepicker.innerWidth()
                      && event.pageY >= datepicker.offset().top && event.pageY <= datepicker.offset().top + datepicker.innerHeight())
                      return;
              }

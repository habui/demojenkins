...
(function ($) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            onMouseDrag: function(data) { return; },
            scrollToToday: true,
            preventDrag: false,
        ...
        var core = {
            ...
            render: function (element) {
                ...
                if (settings.showTotal) {
                    var $totalPanel = core.totalPanel(element);
                ...
                settings.onRender(dicZoneAvaiable, core.drawTimeline);
            },
            ...
            leftPanel: function (element) {
                ...
                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    .append($('<div class="rowbooking spacer"/>')
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        if (entry.desc) {
                            entries.push('<div class="rowbooking name rowbooking' + i + (entry.desc ? "" : " ") + '" id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '" title="' + entry.name + '">' + entry.name + '</span>');
                            entries.push('</div>');
                            entries.push('<div class="rowbooking desc rowbooking' + i + ' " id="RowdId_' + i + '" data-id="' + entry.id + '" data-index="' + i + '" data-name="' + entry.name + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '" title="' + entry.desc + '">' + entry.desc + '</span>');
                            entries.push('</div>');
                        }
                        if (entry.desc.length === 0) {
                            entries.push('<div class="rowbooking name rowbooking' + i + (entry.desc ? "" : " fn-wide") + '" id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '" data-id="' + entry.id + '" data-index="' + i + '" data-name="' + entry.name + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '" title="' + entry.name + '">' + entry.name + '</span>');
                            entries.push('</div>');
                        }
                ...
                if (settings.showTotal) {
                    entries.push('<div class="rowbooking name"><span class="fn-label"></span></div>');
                    entries.push('<div class="rowbooking desc"><span class="fn-label"></span></div>');
                }
                ganttLeftPanel.append(entries.join(""));
                return ganttLeftPanel;
            },
            totalPanel: function (element) {
                /* Left panel */
                var ganttTotalPanel = $('<div class="totalPanel"/>')
                    .append($('<div class="rowbooking spacer"/>')
                    .css("height", tools.getCellSize() * element.headerRows + "px")
                    .css("width", "100%")
                    .css("text-align", "center")
                    .css("line-height", tools.getCellSize() * element.headerRows + "px")
                    .text("COST"));

                var entries = [];
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        entries.push('<div class="rowbooking value rowbooking' + i + (entry.desc ? '' : ' fn-wide') + '" id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '">');
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + entry.total.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '</span>');
                        entries.push('</div>');

                    }
                });
                if (settings.showTotal) {
                    entries.push('<div class="rowbooking value"><span class="fn-label" style="color: #4A3091; font-weight: bold;">' + settings.total.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '</span></div>')
                }
                ganttTotalPanel.append(entries.join(""));
                return ganttTotalPanel;
            },
            drawTimeline: function (zoneid, rowoffset, from, to, share) {
                var id = "selectedtimeline"; var cls = "ganttViolet";
                var desc = "";             
                var dataObj = { zoneId: zoneid, from: from, to: to, share: share };

                // find row                
                var cFrom = 0;var top = 0;var dl = 0;
                ...
                switch (settings.scale) {
                    case "days":
                        ...
                        var dFrom = from;
                        var dTo = to + 86400000;
                        var from = $(chartPtr).find("#dh-" + dFrom);
                        //cFrom = from.attr("offset") - tools.getCellSize();
                        cFrom = from.attr("offset");
                        var topEl = $(chartPtr).find("#rowheader" + rowoffset);
                        dl = Math.floor(((dTo / 1000) - (dFrom / 1000)) / 86400);
                        top = tools.getCellSize() * 4 + 2 + parseInt(topEl.attr("offset"), 10);
                        break;
                }

                var label = (dl > 5)?"Booked:" + share + "%":"["+share + "%]";
                var _bar = core.createProgressBar(dl, id, cls, desc, label, dataObj);
                _bar.css({ 'margin-top': top, 'margin-left': Math.floor(cFrom) });
                dataPanelPtr.append(_bar);
                
            },
            // Create and return the data panel element
            dataPanel: function (element, width) {
                var _bar = null;
                var startCellDrag = null;
                var zoneid = 0;
                var zonename = '';
                var start_time = 0;
                var end_time = 0;
                var currAvailableVal = 0;
                var prevAvailableVal = {};
                ...
                dataPanel.click(function (e) {
                    ...
                    */
                    ...
                    settings.onAddClick(dt, rowId, rowIndex, rowName, e.pageX, e.pageY);
                });

                if (settings.showTotal) {
                    return dataPanel;
                }
                dataPanel.drag("start", function (e) {
                        if (settings.preventDrag) return;
                        var currentCell = tools.getCurrentCell(element, e);
                        startCellDrag = currentCell;
                        currAvailableVal = 100;
                        prevAvailableVal = {};
                        zoneid = currentCell.zoneid;
                        zonename = currentCell.zonename;
                        start_time = parseInt(currentCell.dt);
                        end_time = start_time;
                        if (_bar !== null && _bar.data('booked') === true) {
                            // remove _bar
                            _bar = null;
                        } else if (_bar !== null) {
                            _bar.remove();
                    ...
                    .drag(function (ev) {
                        if (settings.preventDrag) return;
                        var currentCell = tools.getCurrentCell(element, ev);
                        if (currentCell.offset === startCellDrag.offset && currentCell.offset !== undefined) {
                            if (currentCell.dt > startCellDrag.dt) {
                                if (startCellDrag.dt < new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate()).getTime()) {
                                    startCellDrag = currentCell;
                                    return;
                                }
                                end_time = parseInt(currentCell.dt);
                                if (_bar !== null) {
                                    // remove _bar
                                    _bar.remove();
                                }
                                
                                if (prevAvailableVal[currentCell.dt] === null || prevAvailableVal[currentCell.dt] === undefined) {
                                    var availableValue = 100;
                                    var zoneTimelineAvailable = dicZoneAvaiable[currentCell.zoneid];
                                    if (zoneTimelineAvailable !== null && zoneTimelineAvailable !== undefined && Array.isArray(zoneTimelineAvailable)) {
                                        for (var i = 0; i < zoneTimelineAvailable.length; i++) {

                                            var timeframe = zoneTimelineAvailable[i];
                                            if (timeframe.to < currentCell.dt)
                                                continue;
                                            if (currentCell.dt < timeframe.from)
                                                break;
                                            if (timeframe.val !== undefined) {
                                                availableValue = timeframe.val;
                                                break;
                                            }
                                        }
                                    }
                                    currAvailableVal = Math.min(availableValue, currAvailableVal);
                                    prevAvailableVal[currentCell.dt] = currAvailableVal;
                                } else {
                                    currAvailableVal = prevAvailableVal[currentCell.dt];
                                }
                                
                                var id = "selectedtimeline";
                                var cls = "ganttViolet";
                                var desc = "";
                                ...
                                switch (settings.scale) {
                                    case "weeks":
                                        var from = $(element).find("#" + startCellDrag.colid);
                                        cFrom = from.attr("offset");
                                        var to = $(element).find("#" + currentCell.colid);
                                        var cTo = to.attr("offset");

                                        dl = Math.round((cTo - cFrom) / tools.getCellSize()) + 1;
                                        var topEl = $(element).find("#rowheader" + currentCell.offset);
                                        top = tools.getCellSize() * 3 + 2 + parseInt(topEl.attr("offset"), 10);
                                        break;
                                    case "months":
                                        var from = $(element).find("#" + startCellDrag.colid);
                                        ...
                                        var topEl = $(element).find("#rowheader" + currentCell.offset);
                                        dl = Math.round((cTo - cFrom) / tools.getCellSize()) + 1;
                                        top = tools.getCellSize() * 2 + 2 + parseInt(topEl.attr("offset"), 10);
                                        break;
                                    case "hours":
                                        var topEl = $(element).find("#rowheader" + currentCell.offset);
                                        top = tools.getCellSize() * 5 + 2 + parseInt(topEl.attr("offset"), 10);
                                        break;
                                    default:
                                        // days
                                        currentCell.dt = parseInt(currentCell.dt) + 86400000;
                                        end_time = parseInt(currentCell.dt);
                                        var dFrom = startCellDrag.dt;
                                        var dTo = currentCell.dt;
                                ...
                                var label = (dl > 5)?"Available = " + currAvailableVal + "%":currAvailableVal + "%";
                                _bar = core.createProgressBar(dl, id, cls, desc, label, dataObj);
                                _bar.css({ 'margin-top': top, 'margin-left': Math.floor(cFrom) });
                    ...
                    .drag("end", function (e) {
                        if (settings.preventDrag) return;
                        settings.onMouseDrag(_bar, zoneid, zonename, start_time, end_time, currAvailableVal, e.pageX, e.pageY);
                    });
                return dataPanel;
            },
            ...
            rightPanel: function (element, leftPanel) {
                ...
                var yearArr = ['<div class="rowbooking"/>'];
                var daysInYear = 0;

                var monthArr = ['<div class="rowbooking"/>'];
                var daysInMonth = 0;
                ...
                switch (settings.scale) {
                    ...
                    case "hours":
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            if (rm !== month) {
                                monthArr.push(
                                    ('<div class="rowbooking header month" style="width: '
                            ...
                            if (rgetDay !== getDay) {
                                ...
                                dayArr.push('<div class="rowbooking date ' + day_class2 + '" '
                                        + ' style="width: ' + tools.getCellSize() * hoursInDay + 'px;"> '
                                        + ' <div class="fn-label">' + day.getDate() + '</div></div>');
                                dowArr.push('<div class="rowbooking day ' + day_class2 + '" '
                                        + ' style="width: ' + tools.getCellSize() * hoursInDay + 'px;"> '
                            ...
                            horArr.push('<div class="rowbooking day '
                                    + day_class
                                    + '" id="dh-'
                                    + rday.getTime()
                        ...
                        // Last year
                       yearArr.push(
                            '<div class="rowbooking header year" style="width: '
                        ...
                        dayArr.push('<div class="rowbooking date ' + day_class + '" '
                                + ' style="width: ' + tools.getCellSize() * hoursInDay + 'px;"> '
                                + ' <div class="fn-label">' + day.getDate() + '</div></div>');

                        dowArr.push('<div class="rowbooking day ' + day_class + '" '
                                + ' style="width: ' + tools.getCellSize() * hoursInDay + 'px;"> '
                        ...
                        dataPanel.append($('<div class="rowbooking"/>').html(dayArr.join("")));
                        dataPanel.append($('<div class="rowbooking"/>').html(dowArr.join("")));
                        dataPanel.append($('<div class="rowbooking"/>').html(horArr.join("")));
                    ...
                    case "weeks":
                        ...
                        yearArr = ['<div class="rowbooking"/>'];
                        monthArr = ['<div class="rowbooking"/>'];
                        var year = range[0].getFullYear();
                        var month = range[0].getMonth();
                        var day = range[0];
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            // Fill weeks
                            dayArr.push('<div class="rowbooking day wd" '
                    ...
                    case 'months':
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            daysInYear++;
                            monthArr.push('<div class="rowbooking day wd" id="dh-' + tools.genId(rday.getTime()) + '" offset="' + i * tools.getCellSize() + '" repdate="' + rday.genRepDate() + '">' + (1 + rday.getMonth()) + '</div>');
                        }
            ...
            // the entire chart
            createProgressBar: function (days, id, cls, desc, label, dataObj, zoneId, zoneName, startTime, rowIndex, timelineY) {
                ...
                if (settings.showDesciption && desc) {
                    bar
                ...
                bar.click(function (e) {
                    e.stopPropagation();
                    var dt = null;
                    if (timelineY !== undefined && timelineY !== null) {
                        var col = core.elementFromPoint(e.pageX, timelineY);
                        // Was the label clicked directly?
                        if (col.className === "fn-label") {
                            col = $(col.parentNode);
                        } else {
                    ...
                    settings.onItemClick(this, e.pageX, e.pageY, zoneId, zoneName, dt, rowIndex);
                });
            ...
            fillData: function (element, datapanel, leftpanel) {
				...
				var timelineY = datapanel.offset().top;
				switch (settings.scale) {
				    case "weeks":
				        timelineY += tools.getCellSize() * 2;
				        break;
				    case "months":
				        timelineY += tools.getCellSize();
				        break;
				    case "hours":
				        timelineY += tools.getCellSize() * 4;
				        break;
				    case "days":
				        timelineY += tools.getCellSize() * 3;
				        break;
				    default:
				        timelineY += tools.getCellSize() * 2;
				        break;
				...
				$.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        var timelineAvaiable = [];
                        $.each(entry.values, function (j, day) {
                            var _bar = null;

                            // save avaiable index dictionary
                            var startTime = tools.dateDeserialize2(day.from).getTime();
                            var endTime = tools.dateDeserialize2(day.to).getTime();
                            timelineAvaiable.push({from:startTime, to:endTime, val:day.value});

                            switch (settings.scale) {
                                // **Hourly data**
                                case "hours":
                                    var dFrom = tools.genId(tools.dateDeserialize2(day.from).getTime(), element.scaleStep);
                                    var from = $(element).find('#dh-' + dFrom);

                                    var dTo = tools.genId(tools.dateDeserialize2(day.to).getTime(), element.scaleStep);
                                ...
                                // **Weekly data**
                                case "weeks":
                                    var dtFrom = tools.dateDeserialize2(day.from);
                                    var dtTo = tools.dateDeserialize2(day.to);
                                ...
                                // **Days**
                                default:
                                    var dFrom = tools.genId(tools.dateDeserialize2(day.from).getTime());
                                    var dTo = tools.genId(tools.dateDeserialize2(day.to).getTime());

                                    var from = $(element).find("#dh-" + dFrom);
                                    var cFrom = from.attr("offset");

                                    var dl = Math.floor(((dTo / 1000) - (dFrom / 1000)) / 86400);
                                    _bar = core.createProgressBar(
                                                dl,
                                                day.id ? day.id : "",
                                                day.customClass ? day.customClass : "",
                        ...
                        dicZoneAvaiable[entry.id] = timelineAvaiable;
					}
				});
            },
            // **Navigation**
        ...
        var tools = {
            ...
            getMaxDate: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    $.each(entry.values, function (i, date) {
                        maxDate = maxDate < tools.dateDeserialize2(date.to) ? tools.dateDeserialize2(date.to) : maxDate;
                    });
                });
                if (settings.maxDate > 0) {
                    if (maxDate === null || maxDate === undefined)
                        maxDate = tools.dateDeserialize2(settings.maxDate);
                    else
                        maxDate = maxDate < tools.dateDeserialize2(settings.maxDate) ? tools.dateDeserialize2(settings.maxDate) : maxDate;
                }
            ...
            getMinDate: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    $.each(entry.values, function (i, date) {
                        minDate = minDate > tools.dateDeserialize2(date.from) || minDate === null ? tools.dateDeserialize2(date.from) : minDate;
                    });
                });
                if (settings.minDate > 0) {
                    if (minDate === null || minDate === undefined)
                        minDate = tools.dateDeserialize2(settings.minDate);
                    else
                        minDate = minDate > tools.dateDeserialize2(settings.minDate) ? tools.dateDeserialize2(settings.minDate) : minDate;
                }
            ...
            dateDeserialize2: function (timestamp) {
                var date = eval("new Date("+timestamp+")");
                return date;
            },
            ...
            getCellSize: function () {
                if (!tools._getCellSize) {
                    $("body").append(
                        $('<div style="display: none; position: absolute;" class="fn-gantt" id="measureCellWidth"><div class="rowbooking"></div></div>')
                    );
                    tools._getCellSize = $("#measureCellWidth .rowbooking").height();
            ...
            getProgressBarMargin: function () {
                if (!tools._getProgressBarMargin) {
                    $("body").append(
                        $('<div style="display: none; position: absolute;" id="measureBarWidth" ><div class="fn-gantt"><div class="rightPanel"><div class="dataPanel"><div class="rowbooking day"><div class="bar" /></div></div></div></div></div>')
                    );
            ...
            getCurrentCell: function(element, e) {
                var corrX, corrY;
                var leftpanel = $(element).find(".fn-gantt .leftPanel");
                var datapanel = $(element).find(".fn-gantt .dataPanel");
                switch (settings.scale) {
                ...
                var rowIndex = row.data().index;
                var zoneName = row.data().name;
                return { dt: dt, zoneid: zoneId, zonename: zoneName, offset: rowIndex, colid:colid };
            }
        };
